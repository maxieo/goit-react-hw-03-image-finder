  ...
  render() { 
    return (
      <AppStyle>
        ...
        {this.state.status === 'failed' && (<MessageError message={'Sorry, this request is failed '} />)}
        {this.state.images.length > 0 && <ImageGallery results={this.state.images} openModal={ this.openModal} />}
        {this.state.status === 'loading' && <Loader/>}
        {this.state.status === 'resolved' && (<Button loadMore={this.loadMore}></Button>)}
        {this.state.modal && (<Modal largeImageURL={this.state.largeImageURL} tags={this.state.tags} onClose={this.toggleModal} />)}
      </AppStyle>
